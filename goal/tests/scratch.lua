...
local function compileEvent(Tag, evName, data, children, --[[Optional]] optional)
    ...
    -- First, make an appropriate database table:
    local args = {}
    for i,v in ipairs(data) do append(args, v) end
    for i,v in ipairs(children) do append(args, v .. ":INTEGER NOT NULL") end
    for i,v in ipairs(optional) do append(args, v .. ":INTEGER") end

    nodeData(evName) (unpack(args))
    ...
    Event (event "n") (
        Store(evName)(Tag, id "n", location "n", -- Common component
            unpack(tuple_elems)
        )
    )
end
