...
local MethodBuilder = require "flextypes.MethodBuilder"
local StructType = require "flextypes.StructType"
    ...
    for k in values {"local", "elseif", "else", "return", "function", "end", "rawget", "rawset"} do
        for pattern in values {"$", "[%s%(]"} do
            s = s:gsub(k..pattern, function(p1) 
                return p1:gsub(k, function(p2) return p2 --[[AC.WHITE(p2)]] end) 
            end)
        end
    end ; return s
end
